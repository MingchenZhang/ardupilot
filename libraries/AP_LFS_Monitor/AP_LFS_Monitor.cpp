...
void AP_LFS_Monitor::loop()
{
    ...
    while (1) {
        ...
        home_loc.alt += home_height_cm;  // add receiver height to help avoid close by terrain
        ...
        for (; range<distance && i<last_alt_list_size; range += _check_interval_meter, i++) {
            ...
            if (range < 0.01) {
                // at home, get the altitude error
                alt_correction -= terrain_alt_cm - home_height_cm;
            }
            terrain_alt_cm += alt_correction;
        ...
        if (min_ms_to_ground <= 100) {
            gcs().send_text(MAV_SEVERITY_WARNING, "ground reached");
        }else if (min_ms_to_ground < 15000) {
            gcs().send_text(MAV_SEVERITY_WARNING, "%.2f sec to ground", ((ftype)min_ms_to_ground)/1000);
        }
    }
